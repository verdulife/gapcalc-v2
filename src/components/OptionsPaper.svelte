<script lang="ts">
  let { papers, onSave, onCreate, onDelete } = $props();
  ...
  function handleChange(paper: any) {
    onSave({ ...paper });
  }

  async function handleDelete(paper: any) {
    if (!confirm(`¿Eliminar "${paper.label}"?`)) return;
    ...
    {#each papers as paper}
      <li class="flex items-center gap-2">
        ...
        <input
          ...
          step="0.1"
          value={paper.price}
          onchange={(e) => handleChange({ ...paper, price: parseFloat(e.currentTarget.value) })}
